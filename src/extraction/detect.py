...
def _load_aikgraphignore(root: Path) -> list[tuple[Path, str]]:
    """Read .aikgraphignore from root **and ancestor directories**.
    ...
    the .aikgraphignore file was found — so patterns written relative to a
    parent directory still work when aikgraph is run on a subfolder.
    ...
    while True:
        ignore_file = current / ".aikgraphignore"
...
def _is_ignored(path: Path, root: Path, patterns: list[tuple[Path, str]]) -> bool:
    """Return True if path matches any .aikgraphignore pattern."""
    if not patterns:
        return False
    ...
    for anchor, pattern in patterns:
        ...
        try:
            ...
            pass
        # Also try relative to the anchor dir (the .aikgraphignore's location),
...
def detect(
    ...
    *,
            ...
            if p.suffix.lower() in OFFICE_EXTENSIONS:
                ...
                if md_path:
                    ...
                    skipped_sensitive.append(
                        str(p)
                        + " [office conversion failed - pip install aikgraphy[office]]"
                    )
                continue
    ...
    return {
        ...
        "aikgraphignore_patterns": len(ignore_patterns),
    }
