...
from aikgraph.extraction.cache import load_cached, save_cached
...
def _check_tree_sitter_version() -> None:
    ...
    if LANGUAGE_VERSION < 14:
        ...
        raise RuntimeError(
            ...
            f"aikgraph requires tree-sitter >= 0.23.0 (Language API v2). "
...
def extract(paths: list[Path], root: Path | None = None) -> dict:
    ...
    # Parallelize per-file extraction. tree-sitter releases the GIL during parse(),
    # and each _extract_generic call builds its own Parser, so threads are safe.
    from concurrent.futures import ThreadPoolExecutor, as_completed

    max_workers = min((os.cpu_count() or 4), 16)
    aligned: list[dict | None] = [None] * total
    completed = 0
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        futures = {executor.submit(_process, p): i for i, p in enumerate(paths)}
        for fut in as_completed(futures):
            idx = futures[fut]
            aligned[idx] = fut.result()
            completed += 1
            if total >= _PROGRESS_INTERVAL and completed % _PROGRESS_INTERVAL == 0:
                print(
                    f"  AST extraction: {completed}/{total} files "
    ...
    py_paths = [p for p, r in zip(paths, aligned) if p.suffix == ".py" and r is not None]
    if py_paths:
        py_results = [r for p, r in zip(paths, aligned) if p.suffix == ".py" and r is not None]
        try:
...
def collect_files(target: Path, *, follow_symlinks: bool = False, root: Path | None = None) -> list[Path]:
    ...
    from aikgraph.detect import _load_aikgraphignore, _is_ignored
    ignore_root = root if root is not None else target
    patterns = _load_aikgraphignore(ignore_root)
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python -m aikgraph.extract <file_or_dir> ...", file=sys.stderr)
        sys.exit(1)
