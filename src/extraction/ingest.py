...
from aikgraph.utils.security import safe_fetch, safe_fetch_text, validate_url
...
def ingest(
    ...
    """
    Fetch a URL and save it into target_dir as a aikgraph-ready file.
    ...
    try:
        ...
        if url_type == "youtube":
            from aikgraph.transcribe import download_audio
...
def save_query_result(
    ...
    Files are stored in memory_dir (typically aikgraph-out/memory/) with YAML frontmatter
    that aikgraph's extractor reads as node metadata. This closes the feedback loop:
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Fetch a URL into a aikgraph /raw folder"
    )
    ...
    print(f"Ready for aikgraph: {out}")
