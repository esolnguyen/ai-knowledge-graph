...
def _model_name() -> str:
    return os.environ.get("AIKGRAPH_WHISPER_MODEL", _DEFAULT_MODEL)
...
def _get_whisper():
    try:
        ...
        raise ImportError(
            ...
            "Run: pip install 'aikgraphy[video]'"
        ) from exc
...
def build_whisper_prompt(god_nodes: list[dict]) -> str:
    ...
    domain hint from these labels and passes it via AIKGRAPH_WHISPER_PROMPT or
    as initial_prompt — no separate API call needed here.
    """
    if not god_nodes:
        return _FALLBACK_PROMPT

    override = os.environ.get("AIKGRAPH_WHISPER_PROMPT")
    if override:
        return override
