...
def safe_fetch(url: str, max_bytes: int = _MAX_FETCH_BYTES, timeout: int = 30) -> bytes:
    ...
    req = urllib.request.Request(
        url, headers={"User-Agent": "Mozilla/5.0 aikgraph/1.0"}
    )
...
def validate_graph_path(path: str | Path, base: Path | None = None) -> Path:
    ...
    *base* defaults to the `aikgraph-out` directory relative to CWD.
    Also requires the base directory to exist, so a caller cannot
    trick aikgraph into reading files before any graph has been built.
    ...
    if base is None:
        ...
        for candidate in [resolved_hint, *resolved_hint.parents]:
            if candidate.name == "aikgraph-out":
                base = candidate
                break
        if base is None:
    ...
    try:
        ...
        raise ValueError(
            ...
            "Only paths inside aikgraph-out/ are permitted."
        )
